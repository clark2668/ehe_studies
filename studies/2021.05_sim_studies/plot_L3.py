...
cor_weighter = utils_weights.get_weighter(cor_file, 'corsika', 100)
...
numu_mask = numu_npe > 25000
nue_mask = nue_npe > 25000
cor_mask = cor_npe > 25000
...
counts, xedges, yedges, im = ax.hist2d(cor_chisqured[cor_mask], np.log10(cor_npe[cor_mask]), 
		bins=bins,
		weights=cor_weights[cor_mask],
		cmap=cmap,
		norm=colors.LogNorm(),
		)
im.set_clim(1E-5, 1E1)
...
counts, xedges, yedges, im = ax2.hist2d(numu_chisqured[numu_mask], np.log10(numu_npe[numu_mask]), 
		bins=bins,
		weights=numu_atmo_weights[numu_mask],
		cmap=cmap,
		norm=colors.LogNorm(),
		)
im.set_clim(1E-5, 1E1)
...
counts, xedges, yedges, im = ax3.hist2d(nue_chisqured[nue_mask], np.log10(nue_npe[nue_mask]), 
		bins=bins,
		weights=nue_atmo_weights[nue_mask],
		cmap=cmap,
		norm=colors.LogNorm(),
		)
im.set_clim(1E-5, 1E1)
...
plt.tight_layout()
fig.savefig('L3_plots.png', edgecolor='none', bbox_inches='tight')
del fig, ax