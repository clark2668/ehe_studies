...
if do_event_dist:
    ...
    ax.set_ylabel(r'Opening Angle', fontsize=sizer)
    ...
    ax.tick_params(labelsize=sizer)
    ax.set_title('LineFit vs Ophelia')
    ax.plot([-1,1], [-1, 1], 'w:')
    x, y_med, y_lo, y_hi = utils_plot.find_contours_2D(
    ...
    ax2.set_title('Ophelia vs Truth')
    ax2.plot([-1,1], [-1, 1], 'w:')
    x, y_med, y_lo, y_hi = utils_plot.find_contours_2D(
    ...
    ax3.set_title('LineFit vs Truth')
    ax3.plot([-1,1], [-1, 1], 'w:')
    x, y_med, y_lo, y_hi = utils_plot.find_contours_2D(
        x_values=np.cos(true_zenith),
        y_values=np.cos(linefit_zenith),
    ...
    f2_ax.plot(x, y_med - x, 'o', label='LineFit')
